...
const table= $("#table_list").DataTable({
    ...
    columns:[
        ...
        },
        {
            ...
            "sortable":true,
            "render":function(data,type,row,meta){
                return `<img class="img-fluid" src="{{asset('/storage/image/${row.logo}')}}" width="50"> <b> <span onclick="ambilsatudata(${row.ID})">${row.judul}</span>`
            }
        },
        // {
        //     "class":"",
...
const ambilsatudata = (id)=>(
    $.ajax({
        url:`{{url('detailkompetisi')}}?id=${id}`,
        success:function(ress){
            $("#formA [name='id']").val(id)
            $("#formA [name='judul']").val(ress.judul)
            $("#formA [name='logo']").val(ress.logo)
            $("#formA [name='alamat']").val(ress.alamat)
            $("#formA [name='lokasi']").val(ress.lokasi)
...
function submitform(){
    let form = $('#formA');
    const url = "{{ url('addkompetisi') }}"; //menggunakan url yang dibuat, data dapat dilihat pada network.
...
function clearform(){
    $("#formA [name='id']").val('')
    $("#formA [name='judul']").val('')
    $("#formA [name='logo']").val('')
    $("#formA [name='alamat']").val('')
    $("#formA [name='lokasi']").val('')
